...
def callback(data_l, depth_l):
    if data_l.objects:
        ...
        try:
            ...
            angel_x = int(math.degrees(math.acos(pose_r.position.x))) - 90
            print("Angle", angel_x)
...
def handle_get_buttons(req):
    ...
    for pose in pose_array:
        key = ";".join([str(round(pose.position.x,1)), str(round(pose.position.y,1)), str(round(pose.position.z, 1))])
        if not key in d:
            d[key] = []
        d[key].append(pose)
    poses = d[max(d.keys(), key=lambda key_: d[key_])]
    global pose_array
    pose_array = []
    #os.system("rosnode kill /zed_node/left/solo_node_left")
    return GetButtonsResponse(poses[0])
