...
public:
  ...
  void pcCB(const sensor_msgs::PointCloud2ConstPtr &msg)
  {
    ...
    // cloudSegmentation
    // 对非地面点进行简单聚类，为了找 outlier / Simple clustering of non-ground points, in order to find outlier
    for (int i = 0; i < N_SCAN; ++i)
    {
      for (int j = 0; j < Horizon_SCAN; ++j)
      {
    ...
    for (int i = 0; i < N_SCAN; ++i)
    {
      ...
      for (int j = 0; j < Horizon_SCAN; ++j)
      {
        ...
        {
          // TODO: 这里对噪点和地面点的滤波对结果有提升吗？ / Does the filtering of noise and ground points improve the result here?
    ...
    {
      ROS_WARN_STREAM("not complete point cloud received! orientationDiff: " << seg_info_msg_->orientationDiff ); // из-за того, что приходит не полное облако точек (например, оборот в 40 градусов вместо 360 градусов) одометрия начинает вести себя неадекватно
    }
    else
    {
      publish();
  ...
  void labelComponents(int row, int col)
  {
    ...
    que_id_i.push(row);
    que_id_j.push(col);
    line_cnt_flag[row] = true; // 原 LeGO_LOAM 源码中没有 / Not in the original LeGO_LOAM source code
    ...
    while (!que_id_i.empty())
    {
      ...
      for (const auto &iter : neighbor_iter_)
      {
        ...
        {
          // 已访问过，或者是地面/Nan 点 /  Visited, or ground / Nan point
          continue;
        }
        ...
        // 在 seg_alpha_y 上的远处点很难聚类，在近处估计还好 / It is difficult to cluster at far points on seg_alpha_y, it is estimated to be good at close
        // 在 seg_alpha_x 上聚类可以接受 / Clustering on seg_alpha_x is acceptable
