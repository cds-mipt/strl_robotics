...
private:
  ...
  // 回环检测相关 / Loop Loopback detback detection related 
  bool loop_closed_;
  ...
  float history_search_radius_; // 回环检测参数 / Loopback detection parameters
  ...
  // 无回环检测使用 / Use without loopback detection
  ...
  // tf params
  std::string map_frame_, odom_frame_, laser_frame_  ;

public:
  LM(ros::NodeHandle nh, ros::NodeHandle pnh) : nh_(nh), pnh_(pnh)
  {
    onInit();
  }
  void onInit()
  {
    ...
    history_search_radius_ = 10.; // Loop closure parameters
    ...
    if (debug_output) {
      ROS_INFO("LaserMapping onInit end: %.3f ms", t_init.toc());
    }
  }
    ...
    while (ros::ok())
    {
          ...
          std::abs(time_laser_surf_ - time_laser_corner_) < 0.005 && std::abs(time_laser_surf_ - time_laser_outlier_) < 0.005 && std::abs(time_laser_surf_ - time_laser_odom_) < 0.005)
      {
        ...
        if (frame_cnt % 2 == 0)
        {
          ...
          if (debug_output) {
            ROS_INFO("transformAssociateToMap: %.3f ms", t_tatm.toc());
            ROS_INFO("extractSurroundingKeyFrames");
          }
          TicToc t_eskf;
          extractSurroundingKeyFrames();
          if (debug_output) {
            ROS_INFO("extractSurroundingKeyFrames: %.3f ms", t_eskf.toc());
            ROS_INFO("downsampleCurrentScan");
          }
          TicToc t_dcs;
          downsampleCurrentScan();
          if (debug_output) {
            ROS_INFO("downsampleCurrentScan: %.3f ms", t_dcs.toc());
            ROS_INFO("scan2MapOptimization");
          }
          TicToc t_s2mo;
          scan2MapOptimization();
          if (debug_output) {
            ROS_INFO("scan2MapOptimization: %.3f ms", t_s2mo.toc());
          }
          ...
          publish();
          if (debug_output) {
            ROS_INFO("mapping whole time: %.3f ms", t_whole.toc());
          }
        }
        ++frame_cnt;
      }
  ...
  void laserOdomHandler(const nav_msgs::OdometryConstPtr &msg)
  {
    ...
    if (pub_odom_aft_mapped_.getNumSubscribers() > 0)
    {
      ...
      msg->child_frame_id = laser_frame_;
  ...
  void extractSurroundingKeyFrames()
  {
    ...
    else
    {
      ...
      // TODO: 感觉这两步没啥必要，也就是节省了 transformPointCloud 的次数，直接用 surround_keyposes_ds_ 中的不行么 / I feel that these two steps are not necessary, that is, the number of transformPointCloud is saved, can't you use surround_keyposes_ds_ directly?
      // surrounding 中保存的是附近位姿和点云，当做局部的 target 地图来匹配 / The surrounding poses and nearby point clouds are saved as local target maps to match
      // 1. 对于不在当前采样的附近位姿的 surroundingExistingKeyPoses，将其剔除 / 1. Remove surroundingExistingKeyPoses that are not in the vicinity of the current sample
      ...
      // 2. 对于当前采样的附近位姿，如果 surroundingExistingKeyPoses 中没有，就将其添加进去 / For the nearby pose of the current sample, if it is not in surroundingExistingKeyPoses, add it
      for (int i = 0; i < len; ++i)
      {
        bool exist_flag = false;
    ...
    if (debug_output) {
      ROS_INFO("before ds, surf points: %d, corner points: %d", int(surf_from_map_->size()), int(corner_from_map_->size()));
      ROS_INFO("surf points: %d, corner points: %d, time: %.3f ms", int(surf_from_map_ds_->size()), int(corner_from_map_ds_->size()), t_data.toc());
      ROS_INFO("downsize time: %.3f ms", t_ds.toc());
    }
  }
  ...
  void downsampleCurrentScan()
  {
    ...
    *laser_surf_total_ += *laser_outlier_ds_;
    ...
    if (debug_output) {
      ROS_INFO("downsampleCurrentScan: %.3f ms", t_ds.toc());
      ROS_INFO("before downsample: corner size %d, surf size %d", int(laser_corner_->points.size()), int(laser_surf_total_->points.size() + laser_outlier_->points.size()));
      ROS_INFO("after downsample: corner size %d, surf size %d", int(laser_corner_ds_->points.size()), int(laser_surf_total_ds_->points.size()));
    }
  }
  ...
  void scan2MapOptimization()
  {
    ...
    if (debug_output) {
      ROS_INFO("build kdtree time: %.3f ms", t_tree.toc());
    }
        ...
        if (point_dist_[4] < 1.0)
        {
          ...
          if (planeValid)
          {
            ...
            problem.AddResidualBlock(new LidarPlaneCostFunction(cp, norm, negative_OA_dot_norm),
                                     loss_function, params_);
            // TODO: 先解决 corner 数量过少的问题，少了十倍 / First solve the problem of too few corners, ten times less
  ...
  /**
 * @brief 回环检测及位姿图更新 / @brief loopback detection and pose image update
 * ICP 匹配添加回环约束 / ICP matching adds loopback constraint
 */
  ...
  /**
 * @brief 基于里程计的回环检测，直接提取当前位姿附近(radiusSearch)的 keyframe 作为 icp 的 target / Based on the loopback detection of the odometer, directly extract the keyframe near the current pose (radiusSearch) as the target of icp
 * 
 */
  bool detectLoopClosure()
  {
    ...
    for (int i = 0; i < point_idx_.size(); ++i)
    {
      // 时间太短不做回环 / Time is too short to do a loop
    ...
    *latest_keyframe_ += *transformPointCloud(outlier_frames_[latest_history_frame_id_], cloud_keyposes_6d_->points[latest_history_frame_id_]);
    ...
    if (debug_output) {
      ROS_INFO("detectLoopClosure: %.3f ms", t_dlc.toc());
    }
    return true;
  }
  ...
  bool saveMapCB(std_srvs::Empty::Request &req, std_srvs::Empty::Response &res)
  {
    ...
    pcl::io::savePCDFile("/tmp/keypose.pcd", *map_keypose);
    pcl::io::savePCDFile("/tmp/corner.pcd", *map_corner);
    pcl::io::savePCDFile("/tmp/surf.pcd", *map_surf);
    pcl::io::savePCDFile("/tmp/outlier.pcd", *map_outlier);
